...
import com.remitro.transaction.infrastructure.messaging.handler.AccountOpenedEventHandler;
...
public class TransactionEventDispatcher {
	...
	public void dispatch(EventEnvelope eventEnvelope) {
		switch (eventEnvelope.eventType()) {
			case ACCOUNT_OPENED -> accountOpenedEventHandler.handle(
				eventEnvelope.eventId(),
				JsonMapper.fromJSON(eventEnvelope.eventPayload(), AccountOpenedEvent.class)
			);
